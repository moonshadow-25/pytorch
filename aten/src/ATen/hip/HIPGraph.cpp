...
CUDAGraph* CUDAGraph::get_currently_capturing_graph() {
  ...
  auto stream = at::cuda::getCurrentCUDAStream();
  AT_CUDA_CHECK(hipStreamGetCaptureInfo(stream, &status, &current_capture_id));
  TORCH_CHECK(
      status == hipStreamCaptureStatus::hipStreamCaptureStatusActive,
...
std::function<bool(hipStream_t)> CUDAGraph::create_allocate_filter() {
...
std::function<bool(hipStream_t)> CUDAGraph::create_child_allocate_filter() {
  ...
  return [&current_capture_id = conditional_graph_capture_ids_.top()](hipStream_t stream) {
      ...
      AT_CUDA_CHECK(hipStreamGetCaptureInfo(stream, &status, &stream_capture_id));
      return status == hipStreamCaptureStatus::hipStreamCaptureStatusActive && stream_capture_id == current_capture_id;
  };
#else // !defined(USE_ROCM) && (defined(TORCH_HIP_VERSION) && TORCH_HIP_VERSION >= 12040)
  AT_ERROR(
      __func__,
